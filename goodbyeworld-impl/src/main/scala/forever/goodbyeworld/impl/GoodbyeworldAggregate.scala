...
/**
  * This provides an event sourced behavior. It has a state, [[GoodbyeworldState]], which
  ...
  */
object GoodbyeworldBehavior {

  /**
    * Given a sharding [[EntityContext]] this function produces an Akka [[Behavior]] for the aggregate.
    */ 
  def create(entityContext: EntityContext[GoodbyeworldCommand]): Behavior[GoodbyeworldCommand] = {
    ...
    create(persistenceId)
      .withTagger(
        ...
        AkkaTaggerAdapter.fromLagom(entityContext, GoodbyeworldEvent.Tag)
      )
  ...
  private[impl] def create(persistenceId: PersistenceId) = EventSourcedBehavior
      .withEnforcedReplies[GoodbyeworldCommand, GoodbyeworldEvent, GoodbyeworldState](
        persistenceId = persistenceId,
        emptyState = GoodbyeworldState.initial,
...
/**
  ...
  */
case class GoodbyeworldState(message: String, timestamp: String) {
  def applyCommand(cmd: GoodbyeworldCommand): ReplyEffect[GoodbyeworldEvent, GoodbyeworldState] =
    cmd match {
      case x: Hello              => onHello(x)
      case x: UseGreetingMessage => onGreetingMessageUpgrade(x)
    }

  def applyEvent(evt: GoodbyeworldEvent): GoodbyeworldState =
    evt match {
      case GreetingMessageChanged(msg) => updateMessage(msg)
    }
  private def onHello(cmd: Hello): ReplyEffect[GoodbyeworldEvent, GoodbyeworldState] =
    Effect.reply(cmd.replyTo)(Greeting(s"$message, ${cmd.name}!"))

  private def onGreetingMessageUpgrade(
    cmd: UseGreetingMessage
  ): ReplyEffect[GoodbyeworldEvent, GoodbyeworldState] =
    Effect
...
object GoodbyeworldState {

  /**
    * The initial state. This is used if there is no snapshotted state to be found.
    */
  def initial: GoodbyeworldState = GoodbyeworldState("Hello", LocalDateTime.now.toString)
  ...
  /**
    ...
    */
  val typeKey = EntityTypeKey[GoodbyeworldCommand]("GoodbyeworldAggregate")
  ...
  /**
    ...
    */
  implicit val format: Format[GoodbyeworldState] = Json.format
}

/**
  * This interface defines all the events that the GoodbyeworldAggregate supports.
  */
sealed trait GoodbyeworldEvent extends AggregateEvent[GoodbyeworldEvent] {
  def aggregateTag: AggregateEventTag[GoodbyeworldEvent] = GoodbyeworldEvent.Tag
}

object GoodbyeworldEvent {
  val Tag: AggregateEventTag[GoodbyeworldEvent] = AggregateEventTag[GoodbyeworldEvent]
}

/**
  * An event that represents a change in greeting message.
  */
case class GreetingMessageChanged(message: String) extends GoodbyeworldEvent
...
/**
  ...
  */
trait GoodbyeworldCommandSerializable

/**
  * This interface defines all the commands that the GoodbyeworldAggregate supports.
  */
sealed trait GoodbyeworldCommand
    extends GoodbyeworldCommandSerializable
...
/**
  ...
  * application loader.
  */
object GoodbyeworldSerializerRegistry extends JsonSerializerRegistry {
  override def serializers: Seq[JsonSerializer[_]] = Seq(
    // state and events can use play-json, but commands should use jackson because of ActorRef[T] (see application.conf)
    JsonSerializer[GreetingMessageChanged],
    JsonSerializer[GoodbyeworldState],
